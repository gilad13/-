...
# checking if more than 75 precent of a message are words in english
...
def encrypt_pyramid(message, key):
    ...
    :return:        the encrypted string
    """
    if not(checking_parameters(message, key)):
        return False
    # check that is only relevant for encrypt
    for i in message:
        if i == '$':
            print("the message cannot contain the dollar sign")
            return False, "text can not conatain $ sign"
...
def decrypt_pyramid(encrypted_message, key):
    ...
    """
    if not (checking_parameters(encrypted_message, key)):
        return False
    decrypted = ""
...
def checking_parameters(message, key):
    ...
    for i in str(key):
        if not(i.isnumeric()) or int(i) == 0:
            return False, "Key must contain only digits between 1 and 9 (no zeros)."
...
# print("\nhacking:")
# print(hack_pyramid(encrypt_pyramid("how are you this morning ?", 312465)))